...
## Frame (locked for this run): Investor

This deliberation is run from the investor perspective. The decision owner is a prospective seed investor evaluating the offer on the table, $2M for 13.33 percent at a $13M pre-money valuation. The lens is independent advisor to that investor, neutrality-bound, informing an invest, decline, or invest-with-conditions decision and the value-creation and diligence roadmap if the investor proceeds.

The three deliverables are the investor's: a SWOT of LiT as an investment, a Strategy that is the invest, decline, or invest-with-conditions recommendation, and an Execution Plan that is the investor's action plan if proceeding (diligence, deal structure and conditions precedent, milestone-gated capital staging, governance, value creation, monitoring, and exit).

Every finding and recommendation is made from the investor's standpoint. The Round 00 frame-lock step confirms this frame rather than choosing it. Where the evidence would read materially differently for LiT management, note that briefly in the final frame statement only; do not re-argue it throughout.
...
- Treat `background/LiT_Sponsor_Stipulations.md` as binding established facts from Round 00 onward: there is no clean exclusive IP to license (a PCT is pending, not granted); the recovery improvement that drives the revenue model is unvalidated, and validating it is the use of the seed funds; the quantitative inputs are aggressive and compounding; and the NSR royalty is indexed to commodity prices, so the concern is full commodity-price exposure of the royalty base, not the model ignoring price. Reason from these facts and preserve dissent on their implications.
...
- Hold the investor frame throughout.
...
The files in `synthesis/` are professional consulting documents for the prospective investor. They must read as client-grade deliverables, not as a record of this process.
...
- Confirm the investor frame, confirm the council has ingested the sponsor stipulations as binding facts, take the batched reading notes, then consolidate the requests into one register.
